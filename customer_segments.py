...
# **Answer:**
#  Actually, I predict all the features. The Detergents_Paper has the highest R^2 score(0.7286), which may be dropped. The spending on the detergents_papers may highly depend on the spending on the other products which may determine the size of the store. Therefore, it may not be relevant in identifying customer's spending habits. The higher the R2, the more irrelevant it is.
# 
...
# **Answer:**
# patterns of spending instead of actual customers, or customer segments and can both follow (positive values) and go against (negative values) the direction of the feature weights.
# 
...
# 9 clusters: 0.2798
# 
# 2 clusters gives us the highest silhouette score, which is 0.4118.
...
# **Answer:**
# The cluster centers in the graph above are at almost exactly the same value on dimension 2, while being separated in dimension 1. This means that main criteria for separating establishments into categories is how much they spend on "Detergents_Paper", and to a lesser extent, "Milk" and "Grocery". Cluster 0 represents high spending on these categories, while Cluster 1 represents low spending on them.
# 
# This characterization is reinforced when we compare the values of these features in the cluster centers to the medians of the features - for Cluster 0, the values for the "Milk", "Grocery", and "Detergents_Paper" features are much lower than their respective medians (close to or below the 1st quartile for each), whilst for Cluster 1 they are much higher (close to or above the 3rd quartile for each).
# 
# Cluster 0 could possibly represent grocery stores, since they spend much more on things you would buy in a store, whereas Cluster 1 might represent restaurants, since they wouldn't spend as much on groceries, detergents, and paper, but would spend more of their money on fresh food.
...
# **Answer:**
# All the sample points I choose are predicted to be in Cluster 0
# 
